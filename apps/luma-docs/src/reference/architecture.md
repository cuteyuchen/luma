...
- `packages/icons-vue`：Vue 图标组件和响应式适配层，只依赖 `@luma/icons`、Vue 与 Iconify Vue peer。
- `packages/core`：后台核心包，可以依赖 `@luma/icons` 与 `@luma/icons-vue`。
...
- `apps/luma-admin`：Luma 原生写法示例应用，验证 layout、router、permission、theme、dictionary、CRUD 和 request 的组合方式，并通过外链引用独立驾驶舱应用。
...
create-luma-admin  →  生成消费 @luma/core / @luma/icons / @luma/icons-vue 的应用
```
