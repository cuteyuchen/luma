...
常见 Vben Admin 写法到 Luma 原生 API 的迁移层，**不**完整复刻 Vben。
...
| 正式地址（上线后） | `https://www.npmjs.com/package/@luma/vben-compat` |
...
```bash
pnpm add @luma/vben-compat @luma/core vue
```
...
- `useVbenForm`：常用控件映射到 Luma Schema 表单能力
