...
```bash
git clone https://github.com/example/luma.git
cd luma
```
...
并行启动 `luma-mock-api` 与 `luma-admin`。开发期通过 `@luma/vite` 的 **source alias** 直接解析 `packages/*/src`，无需先构建各包。
...
```bash
pnpm --filter luma-cockpit dev
pnpm --filter luma-datav-guide dev
```
...
详见 [脚手架项目](./scaffold) 与 [create-luma-admin](/packages/create-luma-admin)。
...
完整选项见 [@luma/core](/packages/core) 与 [Core API](/reference/core-api)。
