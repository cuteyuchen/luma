...
Luma 中后台管理台示例应用，基于 `@luma/core` 及其配套包搭建，演示登录鉴权、动态菜单、权限指令、SchemaForm/CrudTable、主题定制以及独立驾驶舱外链与 DataV 指南站等完整能力。
...
- Luma 组件库：`@luma/core`、`@luma/charts`、`@luma/icons`、`@luma/icons-vue`
...
- Header 中的驾驶舱入口会打开独立的 [luma-cockpit](../luma-cockpit/README.md) 应用；使用 `pnpm admin:dev` 时需另行启动该应用。
