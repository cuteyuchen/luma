    ...
    for (const layout of layouts) {
      ...
      if (isMobile) {
        ...
        if (layout.value === 'sidebar-nav') {
          ...
          await expect(page.locator('.luma-header__breadcrumb .luma-breadcrumb')).toBeVisible()
          await expect(page.locator('.luma-layout__main > .luma-breadcrumb')).toHaveCount(0)
          ...
          await expect(desktopSidebar).toBeHidden()
          await expect(page.locator('.luma-header__breadcrumb')).toHaveCount(0)
          await expect(page.locator('.luma-layout__main > .luma-breadcrumb')).toBeVisible()
...
test('混合导航会根据一级菜单子级动态显示和隐藏侧栏', async ({ page }) => {
  ...
  const sampledWidths = await page.locator('.luma-header__navigation')
    .getByRole('menuitem', { name: '工作台', exact: true })
    .evaluate(async (element) => {
      const readWidth = (): number => {
        const currentSidebar = document.querySelector<HTMLElement>('.luma-layout__desktop-sidebar')
...
test('宽屏普通页面保持自然高度且卡片内容从顶部排列', async ({ page }) => {
  ...
  const routerView = page.locator('.luma-router-view')
  const workspace = page.locator('.luma-admin-workspace')
  ...
  const quickPage = page.locator('.luma-admin-workspace__columns .luma-page').filter({
    has: page.getByRole('heading', { name: '快捷入口', exact: true }),
  })
  const quickHeaderBox = await quickPage.locator('.luma-page__header').boundingBox()
  const quickBodyBox = await quickPage.locator('.luma-page__body').boundingBox()
  ...
  const overviewPage = page.locator('.luma-admin-example > .luma-page')
