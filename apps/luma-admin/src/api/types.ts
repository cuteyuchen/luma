...
export interface RawSystemMenuRecord { children?: RawSystemMenuRecord[], component: string, externalLink?: string, externalTarget?: '_blank' | '_self', hidden: boolean, icon: string, id: string, name?: string, order: number, parentId: string, path: string, permission?: string, permissions?: string[], redirect?: string, title: string, type: SystemMenuType }
