...
describe('app header actions', () => {
  ...
  it('移动端账户菜单会传送到 body 并复用现有导航事件', () => {
    ...
    expect(dropdown.props('popperClass')).toBe('luma-admin-header-actions__mobile-dropdown')
    ...
    expect(logoutItem.classes()).toContain('luma-admin-header-actions__mobile-danger')
  ...
  it('移动端仅显示两个 36px 入口且桌面动作保持原尺寸', async () => {
    ...
    expect(mobileStyles).toMatch(/\.luma-admin-header-actions__desktop-only\s*{\s*display:\s*none;/)
    expect(mobileStyles).toMatch(/\.luma-admin-header-actions__mobile-menu\s*{\s*display:\s*inline-flex;/)
    expect(mobileStyles).toMatch(/:deep\(\.el-button\)\s*{\s*width:\s*36px;\s*height:\s*36px;/)
    expect(source).toMatch(/\.luma-admin-header-actions__mobile-trigger\s*{[\s\S]*?width:\s*36px;[\s\S]*?height:\s*36px;/)
  })
})
