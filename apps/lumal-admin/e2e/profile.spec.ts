...
test('个人中心可更新资料并校验修改密码表单', async ({ browserDiagnostics, page }) => {
  ...
  await expect(page.locator('.lumal-admin-header-actions__name')).toHaveText('平台管理员')

  await nickname.fill('超级管理员')
  await phone.fill('13800138001')
  await page.getByRole('button', { name: '保存资料' }).click()
  await expect(page.locator('.lumal-admin-header-actions__name')).toHaveText('超级管理员')
