...
import type { PaginationChangePayload, SchemaFormModel } from '@lumal/core/components'
import {
  LumalPage,
  LumalPageLayout,
  LumalPagination,
...
<template>
  <main class="lumal-admin-example">
    <LumalPage title="Page Layout" :description="message">
      <LumalPageLayout @search="handleSearch" @reset="handleReset">
        <template #query>
          <section class="layout-region">
            ...
            </header><LumalSchemaForm v-model="queryModel" :columns="3" :schemas="exampleQuerySchemas" />
          </section>
        </template>
        ...
        <template #toolbar>
          <section class="layout-region">
            ...
            </header><div class="lumal-admin-example__toolbar">
...
.layout-region{padding:14px;border:1px dashed var(--el-border-color);border-radius:8px}.layout-region header{display:flex;align-items:center;gap:10px;margin-bottom:12px;color:var(--el-text-color-secondary);font-size:calc(var(--lumal-font-size-base, 14px) - 1px)}.layout-region--pagination{width:100%}
</style>
