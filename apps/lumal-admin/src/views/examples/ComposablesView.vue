...
import { LumalPage, LumalPagination, LumalSchemaTable } from '@lumal/core/components'
import { useConfirmAction, useDictionaryMap, useFullscreen, usePagination, usePersistentState, useSelection } from '@lumal/core/composables'
import { createStorage } from '@lumal/core/utils'
...
const storage = createStorage(localStorage, 'lumal-example')
...
<template>
  <main class="lumal-admin-example">
    <LumalPage title="Composables 实验区" description="覆盖当前公开的确认、字典、全屏、分页、持久化与选择组合式函数。">
      ...
      <section class="demo-card pagination-card">
        ...
        <LumalSchemaTable :columns="columns" :rows="pagination.items.value" row-key="id" selection :loading="pagination.loading.value" @selection-change="rows => selection.setSelected(rows)" />
        <LumalPagination v-model:page="pagination.page.value" v-model:page-size="pagination.pageSize.value" :total="pagination.total.value" :page-sizes="[5, 10]" @change="({ page, pageSize }) => { pagination.setPageSize(pageSize); pagination.setPage(page) }" />
      </section>
    </LumalPage>
  </main>
</template>
