...
<template>
  <main class="lumal-admin-page lumal-admin-profile">
    <LumalPage title="个人中心" description="维护当前账号的基本资料与登录密码。" :loading="loading">
      <ElAlert
        v-if="loadError"
        :title="loadError"
        type="error"
      ...
      <div v-if="profile" class="lumal-admin-profile__grid">
        <ElCard class="lumal-admin-profile__summary" shadow="never">
          <div class="lumal-admin-profile__avatar" aria-hidden="true">
            {{ userInitial }}
          </div>
        ...
        <ElCard class="lumal-admin-profile__content" shadow="never">
          <ElTabs v-model="activeTab">
            <ElTabPane label="基本资料" name="profile">
              <ElForm
                ref="profileFormRef"
                class="lumal-admin-profile__form"
                :model="profileForm"
                :rules="profileRules"
                label-position="top"
            ...
            <ElTabPane label="修改密码" name="password">
              ...
              >
                ...
                <p class="lumal-admin-profile__hint">
...
.lumal-admin-profile__grid {
  display: grid;
  grid-template-columns: 260px minmax(0, 1fr);
  gap: 16px;
}

.lumal-admin-profile__summary {
  align-self: start;
  text-align: center;
}

.lumal-admin-profile__avatar {
  display: grid;
  width: 72px;
  height: 72px;
  ...
  font-size: calc(var(--lumal-font-size-base, 14px) + 12px);
  font-weight: 700;
}

.lumal-admin-profile__summary h2 {
  margin: 0;
  color: var(--el-text-color-primary);
  font-size: calc(var(--lumal-font-size-base, 14px) + 4px);
}

.lumal-admin-profile__summary p {
  margin: 6px 0 20px;
  color: var(--el-text-color-secondary);
  font-size: calc(var(--lumal-font-size-base, 14px) - 1px);
}

.lumal-admin-profile__summary dl {
  display: grid;
  gap: 12px;
  margin: 0;
  text-align: left;
...
.lumal-admin-profile__summary dl > div {
  display: grid;
...
.lumal-admin-profile__summary dt {
  color: var(--el-text-color-secondary);
  font-size: calc(var(--lumal-font-size-base, 14px) - 1px);
}

.lumal-admin-profile__summary dd {
  min-width: 0;
  margin: 0;
...
.lumal-admin-profile__content {
  min-width: 0;
}

.lumal-admin-profile__form {
  width: min(520px, 100%);
  padding-top: 4px;
}

.lumal-admin-profile__form :deep(.el-button) {
  min-width: 112px;
}

.lumal-admin-profile__hint {
  margin: -2px 0 16px;
  color: var(--el-text-color-secondary);
