  ...
  <main class="lumal-admin-page">
    ...
    >
      <ElAlert
        ...
        :closable="false"
      />
      <div class="lumal-dictionary-manager">
        <section class="lumal-dictionary-manager__types">
          <header class="lumal-dictionary-manager__header">
            <strong>字典类型</strong>
            <ElButton
          ...
          <LumalSchemaTable :columns="typeColumns" :rows="typeRows" row-key="id" action-width="190">
        ...
        <section class="lumal-dictionary-manager__items">
          <header class="lumal-dictionary-manager__header">
            <div>
              <strong>字典项</strong>
              <span>{{ selectedType?.name ?? '请选择字典类型' }}</span>
            </div>
            <div class="lumal-dictionary-manager__actions">
              <ElButton
                v-authority="adminPermissionCodes.systemDictUpdate"
                native-type="button"
          ...
          <LumalSchemaTable :columns="itemColumns" :rows="itemRows" row-key="id" action-width="150">
            <template #actions="{ row }">
              <ElButton
                v-authority="adminPermissionCodes.systemDictUpdate"
                native-type="button"
...
.lumal-dictionary-manager {
  display: grid;
  grid-template-columns: minmax(360px, 0.85fr) minmax(520px, 1.15fr);
  gap: 16px;
}

.lumal-dictionary-manager__types,
.lumal-dictionary-manager__items {
  min-width: 0;
  padding: 16px;
...
.lumal-dictionary-manager__header {
  display: flex;
  gap: 12px;
  align-items: center;
...
.lumal-dictionary-manager__header span {
  margin-left: 8px;
  color: var(--el-text-color-secondary);
  font-size: calc(var(--lumal-font-size-base, 14px) - 1px);
}

.lumal-dictionary-manager__actions {
  display: flex;
  gap: 8px;
}
