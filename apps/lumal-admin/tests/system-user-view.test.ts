import type { SchemaTableRow } from '@lumal/core/components'
import { createAuthorityDirective } from '@lumal/core/directives'
  ...
  return mount(UserView, {
    global: {
      ...
      stubs: {
        ...
        ElTree: TreeStub,
        LumalCrudTable: CrudTableStub,
        LumalSchemaForm: SchemaFormStub,
      },
    },
  })
}
...
describe('system user view', () => {
  ...
  it('移动端默认收起机构面板并在视口切换后保留移动端选择', async () => {
    ...
    expect(wrapper.get('.lumal-admin-user-page__organization-summary').text()).toContain('当前机构：全部机构')
    ...
    expect(wrapper.get('.lumal-admin-user-page__organization-summary').text()).toContain('平台研发中心')
    ...
    await nextTick()
    expect(isPanelShown()).toBe(true)
    expect(wrapper.find('.lumal-admin-user-page__organization-toggle').exists()).toBe(false)
