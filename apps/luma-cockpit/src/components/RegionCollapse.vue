...
const expandIcon = computed(() => (
  props.side === 'left'
    ? (collapsed.value ? 'luma:chevron-right' : 'luma:chevron-left')
    : (collapsed.value ? 'luma:chevron-left' : 'luma:chevron-right')
))
...
<template>
  ...
  >
    ...
    <button
      ...
      @click="toggle"
    >
      <LumaIcon :name="expandIcon" :size="14" />
    </button>
  </div>
</template>
...
.region-collapse__btn {
  ...
  height: 60px;
  padding: 0;
  border: 1px solid color-mix(in srgb, var(--luma-cockpit-border, rgb(6 183 253 / 50%)), transparent 18%);
  border-radius: 4px;
  background: color-mix(in srgb, var(--luma-cockpit-floating-bg, #19374b), transparent 8%);
  color: var(--luma-cockpit-text-secondary, #a8b7c8);
  cursor: pointer;
  transform: translateY(-50%);
  box-shadow: 0 0 12px color-mix(in srgb, var(--luma-cockpit-accent, #00cbf4), transparent 78%);
}
...
.region-collapse__btn:hover {
  border-color: var(--luma-cockpit-accent, #00cbf4);
  color: var(--luma-cockpit-title-text, #fff);
  background: color-mix(in srgb, var(--luma-cockpit-accent, #00cbf4), transparent 84%);
}

.region-collapse__btn:focus-visible {
  outline: 2px solid var(--luma-cockpit-focus-ring, #409eff);
  outline-offset: 2px;
}
</style>
