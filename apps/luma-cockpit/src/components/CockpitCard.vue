...
<template>
  <LumaBorderBox class="standalone-cockpit-card-shell" :variant="8">
    <LumaCockpitCard
...
.standalone-cockpit-card-shell :deep(.luma-border-box__content) {
  display: block;
  width: 100%;
  height: 100%;
  min-height: 0;
...
.standalone-cockpit-card-shell :deep(.luma-cockpit-card) {
  height: 100%;
  border: 0;
...
.standalone-cockpit-card-shell :deep(.luma-cockpit-card::before),
.standalone-cockpit-card-shell :deep(.luma-cockpit-card::after) {
  display: none;
}

.standalone-cockpit-card-shell :deep(.luma-cockpit-card__body) {
  border: 0;
  background: transparent;
}
...
.standalone-cockpit-card__title-content > i {
  width: 16px;
  height: 8px;
  border-left: 2px solid var(--luma-cockpit-accent);
  border-bottom: 1px solid var(--luma-cockpit-accent);
  transform: skewX(-28deg);
  box-shadow: -3px 0 8px color-mix(in srgb, var(--luma-cockpit-accent), transparent 44%);
  flex: none;
}
...
.standalone-cockpit-card__tab-content small {
  margin-left: auto;
  color: color-mix(in srgb, var(--luma-cockpit-accent), transparent 38%);
  font-size: 9px;
  font-weight: 500;
