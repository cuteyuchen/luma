...
const summaryId = `luma-chart-summary-${useId()}`
...
<template>
  <section class="luma-chart-panel" :style="panelStyle">
    <div v-if="$slots.query" class="luma-chart-panel__query">
      <slot name="query" />
    </div>

    <header v-if="title || $slots.actions || showExport || showViewToggle" class="luma-chart-panel__header">
      <h4 v-if="title" class="luma-chart-panel__title">
        {{ title }}
      </h4>
      <div class="luma-chart-panel__actions">
        <slot name="actions" />
        <div v-if="showViewToggle" class="luma-chart-panel__view-toggle" aria-label="图表展示方式">
    ...
    <p v-if="summary" :id="summaryId" class="luma-chart-panel__summary">
      {{ summary }}
    </p>

    <div
      class="luma-chart-panel__body"
      :class="`is-${view}`"
      :style="{ height }"
      :aria-busy="loading"
      :aria-describedby="summary ? summaryId : undefined"
    >
      <div v-if="loading" class="luma-chart-panel__status" role="status">
        <slot name="loading">
          加载中…
        </slot>
      </div>
      <div v-else-if="error" class="luma-chart-panel__status is-error" role="alert">
      ...
      <div v-else-if="noData" class="luma-chart-panel__status" role="status">
        <slot name="empty">
          {{ emptyText }}
        </slot>
      </div>
