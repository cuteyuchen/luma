      ...
      <section v-for="(row, rowIndex) in region.rows" :key="row.id" class="luma-cockpit-designer__grid-row" :style="{ flexBasis: `${row.height}%` }">
          ...
          >
            ...
            >
              ...
              <ElTooltip content="移除模块" placement="top">
                <ElButton type="danger" class="luma-cockpit-designer__remove-widget" data-role="remove-widget" :aria-label="`移除模块 ${cell.widget.title ?? cell.widget.type}`" @click="draft.removeWidget(locationForCell(row, cell.id))">
        ...
        >
          ...
          >
            ...
            <ElTooltip content="移除模块" placement="top">
              <ElButton type="danger" class="luma-cockpit-designer__remove-widget" data-role="remove-widget" :aria-label="`移除 Tab 模块 ${widget.title ?? widget.type}`" @mousedown.stop @click.stop="draft.removeWidget(locationForTabs(row, widget.id))">
                <LumaIcon name="luma:close" :size="12" />
              </ElButton>
            </ElTooltip>
          </article>
