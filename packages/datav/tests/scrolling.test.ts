...
import { h } from 'vue'
import { LumaActiveRingChart, LumaBorderBox, LumaScrollBoard, LumaScrollRankingBoard } from '../src'
...
describe('scroll board', () => {
  ...
  it('最多渲染 visibleRows + 1，按稳定 key 滚动并暴露 slot 参数', async () => {
    ...
    expect(wrapper.findAll('.luma-scroll-board__row')).toHaveLength(3)
    expect(wrapper.text()).toContain('0:第 0 行')
    await vi.advanceTimersByTimeAsync(601)
    expect(wrapper.find('.luma-scroll-board__row').text()).toContain('1:第 1 行')
  ...
  it('悬停时暂停，离开后恢复，并支持分页步长', async () => {
    ...
    await vi.advanceTimersByTimeAsync(601)
    expect(wrapper.find('.luma-scroll-board__row').text()).toContain('第 2 行')
    wrapper.unmount()
  })
  ...
  it('无 slot 时按显式列定义格式化并发出 rowClick', async () => {
    ...
    await wrapper.findAll('.luma-scroll-board__row')[1]!.trigger('click')
    expect(onRowClick).toHaveBeenCalledWith(rows[1], 1)

    await wrapper.findAll('.luma-scroll-board__row')[0]!.trigger('keydown', { key: 'Enter' })
    await wrapper.findAll('.luma-scroll-board__row')[0]!.trigger('keydown', { key: ' ' })
  ...
  it('自定义行使用 list 语义，键盘焦点进入时暂停自动滚动', async () => {
    ...
    expect(wrapper.find('.luma-scroll-board__row').attributes('tabindex')).toBeUndefined()
    ...
    await vi.advanceTimersByTimeAsync(601)
    expect(wrapper.find('.luma-scroll-board__row').text()).toContain('第 1 行')
    wrapper.unmount()
  })
})
...
describe('ranking and autoplay lifecycle', () => {
  ...
  it('ranking 默认降序、限制渲染数量并暴露 ratio', async () => {
    ...
    expect(wrapper.findAll('.luma-scroll-ranking-board__row')).toHaveLength(3)
    expect(wrapper.text()).toContain('0:乙:1')
    await vi.advanceTimersByTimeAsync(601)
    expect(wrapper.find('.luma-scroll-ranking-board__row').text()).toContain('1:丙:0.75')
    wrapper.unmount()
    expect(vi.getTimerCount()).toBe(0)
  })
  ...
  it('ranking 支持 formatter、关闭 autoplay，并保持排名颜色稳定', async () => {
    ...
    const firstRow = wrapper.find('.luma-scroll-ranking-board__row')
    expect(firstRow.text()).toContain('1-b-40台')
    expect(firstRow.attributes('style')).toContain('--luma-ranking-color: hsl(196 82% 58%)')
    await vi.advanceTimersByTimeAsync(900)
    expect(wrapper.find('.luma-scroll-ranking-board__row').text()).toContain('乙')
    expect(formatter).toHaveBeenCalledWith(40, items[1], 0)
    wrapper.unmount()

    const scrollingWrapper = mount(LumaScrollRankingBoard, {
      props: { interval: 300, items, visibleRows: 2 },
    })
    const initialColor = scrollingWrapper.findAll('.luma-scroll-ranking-board__row')
      .find(row => row.text().includes('乙'))!
      .attributes('style')
    await vi.advanceTimersByTimeAsync(1202)
    const scrolledColor = scrollingWrapper.findAll('.luma-scroll-ranking-board__row')
      .find(row => row.text().includes('乙'))!
      .attributes('style')
  ...
  it('页面隐藏、离开视口时暂停，并在卸载时断开观察器', async () => {
    ...
    await vi.advanceTimersByTimeAsync(600)
    expect(wrapper.find('.luma-scroll-ranking-board__label').text()).toBe('乙')
    ...
    observer.emit(true)
    await vi.advanceTimersByTimeAsync(601)
    expect(wrapper.find('.luma-scroll-ranking-board__label').text()).toBe('丙')
    wrapper.unmount()
