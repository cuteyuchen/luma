...
const rootStyle = computed<CSSProperties>(() => ({
  '--lumal-loading-duration': rotationDuration.value,
  '--lumal-loading-primary': primaryColor.value,
  '--lumal-loading-secondary': secondaryColor.value,
  '--lumal-loading-size': cssSize(props.size),
}) as CSSProperties)
</script>
...
<template>
  ...
  >
    <svg
      ...
      ref="svg"
      class="lumal-loading__visual lumal-loading__visual--ring"
      viewBox="0 0 50 50"
      :width="cssSize(size)"
      :height="cssSize(size)"
      aria-hidden="true"
    ...
    <span v-else class="lumal-loading__visual" aria-hidden="true">
      <template v-if="status === 'loading' && variant === 'dots'">
        <i /><i /><i />
      </template>
      <i v-else-if="status === 'loading'" />
      <span v-else class="lumal-loading__state-mark">{{ status === 'success' ? '✓' : '!' }}</span>
    </span>

    <div class="lumal-loading__label">
      <slot>{{ label }}</slot>
    </div>
  </div>
</template>
