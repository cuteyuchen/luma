...
<template>
  <div
    ref="rootRef"
    class="dv-water-pond-level lumal-water-level-pond"
  ...
  >
    ...
    <svg
      ref="svgRef"
      class="lumal-water-level-pond__svg"
    ...
    >
      ...
      <g class="lumal-water-level-pond__water" :clip-path="`url(#${clipId})`">
      ...
      <text class="lumal-water-level-pond__label" :stroke="`url(#${borderGradientId})`" :fill="`url(#${borderGradientId})`" :x="width / 2" :y="height / 2">{{ details }}</text>
      <ellipse v-if="normalizedShape === 'round'" class="lumal-water-level-pond__border" :cx="width / 2" :cy="height / 2" :rx="innerWidth / 2 + 5" :ry="innerHeight / 2 + 5" :stroke="`url(#${borderGradientId})`" />
      <rect v-else class="lumal-water-level-pond__border" x="2" y="2" :rx="normalizedShape === 'roundRect' ? 10 : 0" :width="innerWidth + 12" :height="innerHeight + 12" :stroke="`url(#${borderGradientId})`" />
    </svg>
  </div>
</template>
