...
const tableStyle = computed<CSSProperties>(() => ({
  '--lumal-info-table-columns': String(Math.max(1, props.columns)),
}))
...
<template>
  <dl
    ref="infoTableRef"
    class="lumal-info-table"
    :class="{ 'lumal-info-table--bordered': bordered }"
    :style="tableStyle"
  >
    <div
      v-for="item in visibleItems"
      :key="item.label"
      class="lumal-info-table__item"
      :style="{ gridColumn: `span ${resolveItemSpan(item)}` }"
    >
      <dt class="lumal-info-table__label" :style="labelStyle">
        {{ item.label }}
      </dt>
      <dd class="lumal-info-table__value">
...
.lumal-info-table {
  display: grid;
  grid-template-columns: repeat(var(--lumal-info-table-columns), minmax(0, 1fr));
  margin: 0;
  overflow: hidden;
  border-radius: 8px;
}

.lumal-info-table--bordered {
  border: 1px solid var(--el-border-color-lighter);
}

.lumal-info-table__item {
  display: flex;
  min-width: 0;
