...
export interface CreateLumalAdminProjectOptions {
  apiMode?: 'http' | 'local'
  name: string
  targetDir: string
}

export interface CreateLumalAdminProjectResult {
  targetDir: string
  createdFiles: string[]
}
...
function normalizePackageName(name: string): string {
  ...
  return normalizedName || 'lumal-admin'
}

function resolveProjectName(options: CreateLumalAdminProjectOptions): string {
...
function createMainTs(): string {
  return `import type { IconDefinition } from '@lumal/icons'
...
function createHttpRequestTs(): string {
  return `import { createRequestClient } from '@lumal/core/request'
...
项目通过 \`VITE_API_BASE_URL\` 配置接口地址。业务项目应在应用层定义响应字段、状态码、登录字段和菜单字段映射，不要把业务协议写入 \`@lumal/core\`。
...
const fixedTabs = computed<LumalLayoutTabItem[]>(() => createAdminTabs())
...
function handlePreferencesChange(preferences: LumalPreferences): void {
...
function createRouterTs(): string {
  return `import type { LumalLayoutTabItem } from '@lumal/core/layout'
import type { LumalStaticMenuRecord, MenuRouteRuntime, RouteRegistryRouterLike, SidebarMenuItem } from '@lumal/core/router'
import type { Router, RouteRecordRaw, RouterHistory } from 'vue-router'
import { setupPermissionGuard } from '@lumal/core/permission'
...
/***********************菜单配置*********************/
export const adminRouteRecords: LumalStaticMenuRecord[] = [
  {
...
function flattenMenuTabs(menus: SidebarMenuItem[]): LumalLayoutTabItem[] {
...
/**
 * 路由驱动模式下的固定页签：仅返回首页，其余页签由 LumalLayout 依据
 * routeTabResolver 在导航时动态追加，并按 tabStorageKey 做会话级持久化。
 */
export function createAdminTabs(): LumalLayoutTabItem[] {
...
<template>
  <main class="lumal-admin-home">
    <LumalPage
      title="工作台"
      description="这是脚手架生成的轻量后台首页，可替换为你的业务指标和快捷入口。"
    >
      <section class="lumal-admin-home__metrics" aria-label="工作台指标">
        <article v-for="metric in metrics" :key="metric.label" class="lumal-admin-home__metric">
...
<template>
  <main class="lumal-admin-page">
    ...
    >
      <ul class="lumal-admin-page__list">
        <li
          v-for="feature in features"
          :key="feature"
          class="lumal-admin-page__item"
        >
          {{ feature }}
        </li>
      </ul>
...
<template>
  <main class="lumal-admin-page">
    <LumalPage title="页面不存在" description="请检查地址，或返回工作台继续操作。">
...
const form = reactive({ username: 'admin', password: 'lumal123' })
...
<template>
  <main class="lumal-admin-login">
    <section class="lumal-admin-login__panel">
      <div class="lumal-admin-login__brand" aria-hidden="true">L</div>
      <h1>Lumal Admin</h1>
      <p>轻量后台基座</p>
      ...
      <small>演示账号：admin / lumal123</small>
    </section>
  </main>
</template>
`
...
function createAccessTs(): string {
  return `import { createPermissionStore } from '@lumal/core/permission'
...
function createSessionTs(): string {
  return `import { createAuthSession } from '@lumal/core/auth'
...
export async function login(payload: LoginPayload): Promise<AdminUser> {
  ...
  if (payload.username !== 'admin' || payload.password !== 'lumal123') {
    throw new Error('用户名或密码错误')
  }

  const user = { name: '管理员', username: payload.username }
  session.setSession({ accessToken: 'lumal-demo-token' })
...
function createPreferencesTs(): string {
  return `import type { LumalPreferencesDefaults } from '@lumal/core/theme'
...
} satisfies LumalPreferencesDefaults

const preferencesStore = createPreferencesStore({
  defaults: adminPreferenceDefaults,
  storage: localStorage,
  storageKey: 'lumal-admin:preferences',
})
...
function createSettingsDrawerVue(): string {
  return `<script setup lang="ts">
import type { LumalPreferences, LumalPreferencesDefaults } from '@lumal/core/theme'
...
function createRequestExampleTs(): string {
  return `import type { RequestContext } from '@lumal/core/request'
...
.lumal-request-panel {
  ...
  box-shadow: var(--lumal-shadow-card);
}
...
.lumal-admin-home {
  display: grid;
  gap: var(--lumal-page-gutter, 20px);
  box-sizing: border-box;
  padding: var(--lumal-page-gutter, 20px);
}

.lumal-admin-home__metrics {
  display: grid;
  grid-template-columns: repeat(4, minmax(0, 1fr));
  gap: 16px;
}

.lumal-admin-home__metric {
  display: grid;
  min-width: 0;
  gap: 8px;
  padding: 18px;
...
.lumal-admin-home__metric strong {
  color: var(--el-color-primary);
  font-size: 28px;
}

.lumal-admin-home__metric small {
  color: var(--el-text-color-secondary);
}
...
.lumal-admin-header-actions .el-button {
  min-width: 44px;
  min-height: 44px;
  margin: 0;
}

.lumal-admin-header-actions span {
  max-width: 120px;
  overflow: hidden;
  color: var(--el-text-color-regular);
  font-size: 13px;
...
.lumal-admin-login__panel {
  ...
  padding: 32px;
  border: 1px solid var(--el-border-color-lighter);
  border-radius: var(--lumal-radius-large, 12px);
  background: var(--el-bg-color);
  box-shadow: var(--lumal-shadow-dialog);
}

.lumal-admin-login__brand {
  display: grid;
  width: 48px;
  height: 48px;
  place-items: center;
...
.lumal-admin-login h1 {
  margin: 20px 0 6px;
}

.lumal-admin-login p,
.lumal-admin-login small {
  color: var(--el-text-color-secondary);
}

.lumal-admin-login .el-form {
  margin: 24px 0 16px;
}

.lumal-admin-login .el-button {
  width: 100%;
  min-height: 44px;
}
