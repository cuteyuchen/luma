...
应用负责接口地址、字段映射、业务状态码和错误文案；Core 只处理 `ApiEnvelope<T>`、`PageResult<T>`、`AuthSessionData` 等标准模型、通用状态和可组合行为。Admin 的非标准响应、分页、菜单和 Token 字段集中在 `src/api/adapters.ts`。Admin 登录后通过菜单 API 加载标准菜单，使用 route registry 动态注册路由；首次守卫初始化复用同一个 Promise。会话清理通过应用侧重置回调统一清除权限、菜单、动态路由和标签，避免 Core 反向依赖 Admin 路由实现。
